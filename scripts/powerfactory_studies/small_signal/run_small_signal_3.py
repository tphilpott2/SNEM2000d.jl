...
# output directory
output_dir = (
    path_nem20000d / "results" / "powerfactory" / "small_signal" / "small_signal_3"
)

nsw_gen_unstable_hours = [46, 47, 54, 55, 61, 101, 102, 110, 111, 142]
...
if __name__ == "__main__":
    ...
    # get no PSS scenarios folder
    stage_2_folder = op_scens.GetContents("ss_stage_2.IntFolder")[0]

    # create stage 3 folder
    if len(op_scens.GetContents("ss_stage_3.IntFolder")) != 0:
        stage_3_folder = op_scens.GetContents("ss_stage_3.IntFolder")[0]
    else:
        stage_3_folder = op_scens.CreateObject("IntFolder")
        stage_3_folder.loc_name = "ss_stage_3"

    # run for each operation scenario
    for op_scen in stage_2_folder.GetContents():
        interval_name = op_scen.loc_name
        interval_name = interval_name.replace("_ss_stage_2", "")
        ...
        # create ss_stage_3 scenario
        stage_3_scen = stage_3_folder.AddCopy(op_scen)
        stage_3_scen.loc_name = f"{interval_name}_ss_stage_3"
        stage_3_scen.Activate()

        # turn off PSS's for selected gens
        if hour_int in nsw_gen_unstable_hours:
            turn_off_PSSs(other_unstable_gen_list_nsw)
